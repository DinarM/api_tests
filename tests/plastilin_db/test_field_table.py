...
class TestFieldTableDelete:
    """
    Тесты для удаления таблицы полей
    """

    def test_delete_field_table_success(
        self, get_token, plastilin_db_api, schema_validator, data_helper
    ):
        """
        Тест успешного удаления таблицы полей
        """
        token = get_token('company_1.head_of_company')
        spec_id = data_helper.get_or_create_spec_id_by_name(
            token=token, russian_name=TEST_CULTURES['wheat']['russian_name']
        ...
        response = plastilin_db_api.delete_field_table(
            token=token, field_id=response.json()['field_id']
        )
