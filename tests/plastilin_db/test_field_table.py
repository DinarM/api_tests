...
from utils.api.constants import TEST_CULTURES, REGIONS

class TestFieldTableGet:
    """
    Тесты для получения таблицы полей
    """
    def test_get_field_table_success(self, get_token, plastilin_db_api, schema_validator, data_helper):
        """
        ...
        schema_validator.assert_valid_response(
            response_data, 
            'plastilin_db/field_table/get_response.json'
        )
...
class TestFieldTableCreate:
    ...
    """
    def test_create_field_table_success(self, get_token, plastilin_db_api, schema_validator, data_helper):
        """
        ...
        response = plastilin_db_api.create_field_table(token=token, spec_id=spec_id, field_name=field_name, region=REGIONS['Амурская область'])
        assert response.status == HTTPStatus.OK
        response_data = response.json()
        schema_validator.assert_valid_response(
            response_data, 
            'plastilin_db/field_table/create_response.json'
        )