...
class TestFieldMapColoringDataGet:
    def test_field_map_coloring_data_get_success(
        ...
        response = plastilin_db_api.get_field_map_coloring_data(
            token=token, year_id=year_id, field_id=field_id
        )
        assert response.status == HTTPStatus.OK
        response_data = response.json()
        schema_validator.assert_valid_response(
            response_data, 'plastilin_db/field_map_coloring_data/get_response.json'
        )
