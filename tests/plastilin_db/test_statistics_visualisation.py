...
class TestStatisticsVisualisation:
    def test_statistics_visualisation_success(
        ...
        response = plastilin_db_api.statistics_visualisation(
            token=token,
            year_id=year_id,
        )
        ...
        schema_validator.assert_valid_response(
            response_data,
            'plastilin_db/statistics_visualisation/get_response.json'
        )